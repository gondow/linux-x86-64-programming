...
- `%rax`レジスタに❶ `0x1122334455667788`を入れると，
  当たり前ですが`%rax`レジスタには❷`0x1122334455667788`が入っています．
- `%eax`には(`%rax`の下位4バイトなので)❸ `0x55667788`が入っています．
- `%ax`には(`%rax`の下位2バイトなので)❹ `0x7788`が入っています．
- `%ah`には(`%ax`の上位1バイトなので)❺`0x77`が入っています．
- `%al`には(`%ax`の下位1バイトなので)❻`0x88`が入っています．
